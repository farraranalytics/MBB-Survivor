  ...
  return (
    <div className="min-h-screen bg-[#0D1B2A] flex flex-col items-center justify-center px-5 py-12">
      <div className="max-w-md w-full text-center animate-fade-in">
        ...
        {/* Subtext */}
        <p className="text-[#9BA3AE] text-sm mb-6 leading-relaxed" style={{ fontFamily: "'DM Sans', sans-serif" }}>
          Pools are locked for the 2026 tournament, but the action is live.
        </p>

        {/* Primary CTA — Bracket Planner */}
        <div className="mb-6">
          <Link
            href="/analyze"
            className="block w-full py-4 rounded-[10px] btn-orange font-bold text-base text-center"
            style={{ fontFamily: "'Oswald', sans-serif", letterSpacing: '0.05em', textTransform: 'uppercase' }}
          >
            Explore the Bracket Planner →
          </Link>
          <p className="text-xs text-[#9BA3AE] mt-2" style={{ fontFamily: "'DM Sans', sans-serif" }}>
            Plan your strategy, predict upsets, and track every game — no pool required.
          </p>
        </div>

        {/* Secondary CTAs based on auth state */}
        {user ? (
          <div className="space-y-3 mb-8">
            <Link
              href="/dashboard"
              className="block w-full py-3.5 rounded-[10px] btn-secondary font-bold text-sm text-center"
